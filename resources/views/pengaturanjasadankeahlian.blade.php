...
@section('content')
    <div class="settings-page">
        <div class="container">
            ...
            {{-- Alerts handled by parent layout (app.blade.php) --}}

            @php
                $hasilpotongan3 = explode("~", Auth::user()->tukang->pengalamanbekerja ?? '');
            @endphp
                                        ...
                                        <div class="service-item" id="kurikulumrow{{ $i + 1 }}">
                                            <div class="service-info">
                                                ...
                                                <span class="service-price">Rp
                                                    {{ number_format($value->biayajasatersedia, 0, ',', '.') }}</span>
                                            </div>
                                            <button type="button" class="btn-remove hapuskurikulum" id="{{ $i + 1 }}">
                                                <i class="fas fa-trash"></i>
                                            </button>
                                            <input id="kurikulumhidden{{ $i + 1 }}" type="hidden"
                                                value="{{ $value->biayajasatersedia }}" name="kurikulum[]">
                                            <input id="kurikulumhiddenjp{{ $i + 1 }}" type="hidden"
                                                value="{{ $value->id_jenispemesanan }}" name="kurikulumjp[]">
                                        </div>
                                        @php $i++; @endphp
                                    @endforeach
                                </div>
                                ...
                                <div id="kurikulumdiv2" class="service-items">
                                    ...
                                    @foreach($jasatersediaborongan as $value)
                                        <div class="service-item" id="kurikulumrow2{{ $i + 1 }}">
                                            ...
                                            <input id="kurikulumhidden2{{ $i + 1 }}" type="hidden"
                                                value="{{ $value->biayajasatersedia }}" name="kurikulum2[]">
                                            <input id="kurikulumhiddenjp2{{ $i + 1 }}" type="hidden"
                                                value="{{ $value->id_jenispemesanan }}" name="kurikulumjp2[]">
                                        </div>
                                        @php $i++; @endphp
                                    @endforeach
                                </div>
                ...
                <div class="settings-card animate-fadeIn stagger-1">
                    ...
                    <div class="card-body">
                        <div class="add-exp-row">
                            <input type="text" class="form-input" id="kurikulumbox3"
                        ...
                        <div id="kurikulumdiv3" class="exp-list">
                            @if(Auth::user()->tukang && Auth::user()->tukang->pengalamanbekerja != "")
                                @for($i = 0; $i < count($hasilpotongan3); $i++)
                                    <div class="exp-item" id="kurikulumrow3{{ $i + 1 }}">
                                        ...
                                        <input id="kurikulumhidden3{{ $i + 1 }}" type="hidden" value="{{ $hasilpotongan3[$i] }}"
                                            name="kurikulum3[]">
                                    </div>
                                @endfor
                            @endif
                    ...
                    <div class="card-body">
                        <div class="form-field">
                            ...
                            <select class="form-input" name="pengalaman">
                                @for($i = 1; $i <= 50; $i++)
                                    <option {{ (Auth::user()->tukang->lamapengalamanbekerja ?? 0) == $i ? 'selected' : '' }}
                                        value="{{ $i }}">
                                        {{ $i }} Tahun
                                    </option>
                                @endfor
                        ...
                        <div class="form-field">
                            ...
                            <textarea class="form-input" name="deskripsi" rows="4"
                                placeholder="Jelaskan keahlian dan spesialisasi Anda...">{{ Auth::user()->tukang->deskripsikeahlian ?? '' }}</textarea>
                        </div>
                    </div>
                </div>
    ...
    <style>
        ...
        .add-service-row .form-input {
            flex: 1;
            min-width: 150px;
        }

        .add-exp-row .form-input {
            flex: 1;
        }
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            let countExp = {{ (Auth::user()->tukang && Auth::user()->tukang->pengalamanbekerja) ? count(explode('~', Auth::user()->tukang->pengalamanbekerja)) : 0 }};
