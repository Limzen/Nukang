...
@section('content')
    <div class="request-page">
        <div class="container">
            ...
            @if(count($pesanan) > 0)
                <div class="active-orders animate-fadeIn">
                    <div class="orders-header">
                        <h3><i class="fas fa-calendar-check"></i> Pesanan Aktif</h3>
                        <span class="orders-count">{{ count($pesanan) }} pesanan</span>
                    </div>
                        ...
                        <table>
                            ...
                            <tbody>
                                ...
                                @foreach($pesanan as $value)
                                    <tr>
                                        <td>{{ $i }}</td>
                                        <td><span class="order-number">{{ $value->nomorpemesanan }}</span></td>
                            ...
                            <div class="request-footer">
                                <form action="{{ url('permintaan-pesanan/' . $value->id_pemesanan . '/tolak') }}" method="POST"
                                    ...
                                    <div class="reject-reason">
                                        ...
                                        <textarea name="alasanpenolakan" placeholder="Masukkan alasan jika menolak..."></textarea>
                                    </div>
                                </form>
                                <div class="action-buttons">
                                    <form action="{{ url('permintaan-pesanan/' . $value->id_pemesanan . '/tolak') }}" method="POST"
                                        id="reject-form-{{ $value->id_pemesanan }}">
                                        @csrf
                                        <input type="hidden" name="alasanpenolakan" id="alasan-{{ $value->id_pemesanan }}">
                                        <button type="button" class="btn btn-danger"
                                            onclick="submitReject({{ $value->id_pemesanan }})">
                                            <i class="fas fa-times"></i> Tolak
                                        </button>
                                    </form>
                                    <form action="{{ url('permintaan-pesanan/' . $value->id_pemesanan . '/terima') }}"
                                        method="POST">
                                        @csrf
    ...
    <script>
        function submitReject(id) {
            // Get the value from the visible textarea in the reject-form class
            const card = document.getElementById('reject-form-' + id).closest('.request-card');
            const textarea = card.querySelector('.reject-reason textarea');
            const hiddenInput = document.getElementById('alasan-' + id);
