...
.content-card { background: var(--bg-card); border: 1px solid var(--border-primary); border-radius: var(--radius-xl); overflow: hidden; margin-bottom: var(--space-6); }
.content-card .card-header { display: flex; align-items: center; gap: var(--space-4); padding: var(--space-5); background: var(--bg-tertiary); border-bottom: 1px solid var(--border-primary); }
.content-card .card-icon { width: 48px; height: 48px; background: var(--gradient-primary); border-radius: var(--radius-lg); display: flex; align-items: center; justify-content: center; color: white; font-size: 1.25rem; }
.content-card .card-title { flex: 1; }
.content-card .card-title h3 { font-size: 1.1rem; margin-bottom: var(--space-1); }
.content-card .card-title p { font-size: 0.85rem; color: var(--text-tertiary); margin: 0; }
.content-card .card-body { padding: var(--space-6); }
.location-map { height: 400px; border-radius: var(--radius-lg); overflow: hidden; border: 1px solid var(--border-primary); margin-bottom: var(--space-4); }
.map-controls { display: flex; align-items: center; justify-content: space-between; padding: var(--space-4); background: var(--bg-tertiary); border-radius: var(--radius-lg); }
.distance-info { display: flex; align-items: center; gap: var(--space-3); }
.distance-info i { color: var(--success); font-size: 1.25rem; }
.distance-info span { font-weight: 600; color: var(--text-primary); }
.location-cards { display: grid; grid-template-columns: 1fr 1fr; gap: var(--space-4); margin-bottom: var(--space-6); }
.location-card { display: flex; align-items: center; gap: var(--space-4); padding: var(--space-5); background: var(--bg-card); border: 1px solid var(--border-primary); border-radius: var(--radius-xl); }
.loc-icon { width: 48px; height: 48px; background: var(--gradient-accent); border-radius: var(--radius-lg); display: flex; align-items: center; justify-content: center; color: white; font-size: 1.25rem; }
.loc-icon-green { background: var(--gradient-primary); }
.loc-info { flex: 1; }
.loc-label { display: block; font-size: 0.8rem; color: var(--text-tertiary); margin-bottom: var(--space-1); }
.loc-value { font-weight: 500; color: var(--text-primary); }
...
@media (max-width: 768px) { 
    ...
    .map-controls { flex-direction: column; gap: var(--space-3); }
    .cta-section { flex-direction: column; text-align: center; gap: var(--space-4); } 
}
</style>
...
setTimeout(function() { pilihLokasi(); }, 1000);
...
function initMapDatabase() {
    uluru = { lat: parseFloat(document.getElementById('latitudesivendor').value), lng: parseFloat(document.getElementById('longtitudesivendor').value) };
...
function initMap() {
    ...
    document.getElementById("get").onclick = function() {
        if (this.innerText.includes("Tampilkan Rute")) { calculateRoute(); this.innerHTML = '<i class="fas fa-map"></i> Tampilkan Biasa'; }
        else { initMap(); this.innerHTML = '<i class="fas fa-directions"></i> Tampilkan Rute'; }
    };
}
...
function calculateRoute() {
    ...
    var request = { origin: uluru, destination: uluruclient, travelMode: 'DRIVING' };
    directionService.route(request, function(result, status) { if (status === "OK") { directionDisplay.setDirections(result); } });
}
...
<script src="https://maps.googleapis.com/maps/api/js?key=AIzaSyDVHbAYfrB3OBft96wmCAmmxYJferc_Gz0" async defer></script>
@endsection