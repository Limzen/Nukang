...
@section('content')
	...
	<style>
		...
		/* Modal Styling for Progress */
		#modalTambahProgress .modal-dialog,
		.modal-dialog.modal-lg {
			max-width: 700px;
			margin: 30px auto;
		}

		#modalTambahProgress .modal-content {
			background: var(--bg-card);
			border: 1px solid var(--border-primary);
			border-radius: var(--radius-xl);
		...
		#modalTambahProgress .modal-header {
			padding: var(--space-5);
			border-bottom: 1px solid var(--border-primary);
			background: var(--bg-tertiary);
			border-radius: var(--radius-xl) var(--radius-xl) 0 0;
		...
		#modalTambahProgress .modal-body {
			padding: var(--space-6);
			min-height: 400px;
			max-height: 70vh;
			overflow-y: auto;
		...
		#modalTambahProgress .modal-footer {
			padding: var(--space-4) var(--space-5);
			border-top: 1px solid var(--border-primary);
			background: var(--bg-tertiary);
			display: flex;
		...
		#modalTambahProgress .form-group {
			margin-bottom: var(--space-5);
		}

		#modalTambahProgress .form-group label {
			display: block;
			font-weight: 600;
			margin-bottom: var(--space-2);
			color: var(--text-primary);
		...
		#modalTambahProgress .form-control {
			width: 100%;
			padding: var(--space-3);
			background: var(--bg-tertiary);
			border: 1px solid var(--border-primary);
		...
		#modalTambahProgress .form-control:focus {
			outline: none;
			border-color: var(--success);
			box-shadow: 0 0 0 3px rgba(16, 185, 129, 0.1);
		}

		#modalTambahProgress textarea.form-control {
			resize: vertical;
			min-height: 100px;
		}
				...
				<div class="provider-card-inner">
					...
					<div class="provider-details">
						...
						@else
							...
							<div class="rating-stars" style="justify-content: flex-start; margin: var(--space-2) 0;">
								...
								@endfor
								@if($value->jumlahvote)
									<span class="rating-text">({{ $value->jumlahvote }} ulasan)</span>
								@endif
							</div>
					...
					@if(count($laporanprogress) > 0 || Auth::user()->statuspengguna == '2')
						<button type="button" class="btn-premium btn-premium-primary" data-toggle="modal"
							data-target="#trackingProgressModal" onclick="$('#trackingProgressModal').addClass('show in')">
					...
					@if(Auth::user()->statuspengguna == '1' && $statuspemesanan == '3' && 
					   (($value->kategoripemesanan == '0' && $value->tanggalbekerja >= date('Y-m-d')) || 
					    ($value->kategoripemesanan == '1' && $value->tanggalselesai >= date('Y-m-d'))))
...
<div class="price-card">
	...
	<div class="modal fade" id="trackingProgressModal" tabindex="-1" role="dialog" style="z-index: 1050;">
		<div class="modal-dialog" role="document" style="width: 90%; max-width: 800px; margin: 50px auto;">
			<div class="modal-content" style="background: var(--bg-card); border: 1px solid var(--border-primary); border-radius: 16px; box-shadow: 0 25px 50px -12px rgba(0,0,0,0.5);">
				<div class="modal-header" style="padding: 20px 24px; border-bottom: 1px solid var(--border-primary); background: var(--bg-tertiary); border-radius: 16px 16px 0 0; display: flex; justify-content: space-between; align-items: center;">
					<h4 class="modal-title" style="margin: 0; font-size: 1.25rem; display: flex; align-items: center; gap: 10px;">
						<i class="fas fa-chart-line" style="color: var(--success);"></i> Tracking Progress Pekerjaan
					</h4>
					<button type="button" class="close" onclick="closeTrackingModal()" style="background: var(--bg-secondary); border: none; width: 36px; height: 36px; border-radius: 8px; font-size: 1.5rem; color: var(--text-secondary); cursor: pointer;">&times;</button>
				</div>
...
onclick="showDeleteProgressModal('delete-form-progress-{{ $progress->id_progress }}', '{{ \Carbon\Carbon::parse($progress->tanggal_progress)->format("d M Y H:i") }}')"
style="padding: var(--space-2) var(--space-3); font-size: 0.8rem; color: #ef4444;">
<i class="fas fa-trash"></i> Hapus
</button>
<form id="delete-form-progress-{{ $progress->id_progress }}" 
action="{{ url('progress/' . $progress->id_progress . '/delete') }}" method="POST" style="display: none;">
<input type="hidden" name="_token" value="{{ csrf_token() }}">
</form>
										</div>
									@endif
				...
				<div class="modal-footer" style="padding: 16px 24px; border-top: 1px solid var(--border-primary); background: var(--bg-tertiary); border-radius: 0 0 16px 16px;">
					<button type="button" onclick="closeTrackingModal()" style="padding: 12px 24px; background: var(--bg-secondary); border: 1px solid var(--border-primary); border-radius: 8px; color: var(--text-secondary); font-weight: 600; cursor: pointer;">Tutup</button>
				</div>
			</div>
		</div>
	</div>
	...
	@if(Auth::user()->statuspengguna == '2')
		<div class="modal fade" id="modalTambahProgress" tabindex="-1" style="z-index: 1060;">
			<div class="modal-dialog" style="width: 90%; max-width: 800px; margin: 50px auto;">
				<div class="modal-content" style="background: var(--bg-card); border: 1px solid var(--border-primary); border-radius: 16px; box-shadow: 0 25px 50px -12px rgba(0,0,0,0.5);">
					<form action="{{ url('riwayatpemesanan/' . $value->id_pemesanan . '/store') }}" method="POST"
						...
						<div class="modal-header" style="padding: 20px 24px; border-bottom: 1px solid var(--border-primary); background: var(--bg-tertiary); border-radius: 16px 16px 0 0; display: flex; justify-content: space-between; align-items: center;">
							<h4 class="modal-title" style="margin: 0; font-size: 1.25rem; display: flex; align-items: center; gap: 10px;">
								<i class="fas fa-plus-circle" style="color: var(--success);"></i> Tambah Progress
							</h4>
							<button type="button" class="close" onclick="closeTambahModal()" style="background: var(--bg-secondary); border: none; width: 36px; height: 36px; border-radius: 8px; font-size: 1.5rem; color: var(--text-secondary); cursor: pointer;">
								&times;
							</button>
						</div>
						...
						<div class="modal-body" style="padding: 24px; max-height: 60vh; overflow-y: auto;">
							<div class="form-group" style="margin-bottom: 20px;">
								<label style="display: block; font-weight: 600; margin-bottom: 8px; color: var(--text-primary);">Tanggal <span style="color: #ef4444;">*</span></label>
								<input type="datetime-local" name="tanggal_progress" class="form-control" required 
									style="width: 100%; padding: 12px; background: var(--bg-tertiary); border: 1px solid var(--border-primary); border-radius: 8px; color: var(--text-primary); font-size: 1rem;">
							</div>

							<div class="form-group" style="margin-bottom: 20px;">
								<label style="display: block; font-weight: 600; margin-bottom: 8px; color: var(--text-primary);">Isi Kegiatan <span style="color: #ef4444;">*</span></label>
								<textarea name="informasi_pekerjaan" class="form-control" rows="4"
									placeholder="Deskripsikan progress pekerjaan..." required
									style="width: 100%; padding: 12px; background: var(--bg-tertiary); border: 1px solid var(--border-primary); border-radius: 8px; color: var(--text-primary); font-size: 1rem; resize: vertical; min-height: 120px;"></textarea>
							</div>

							<div class="form-group" style="margin-bottom: 20px;">
								<label style="display: block; font-weight: 600; margin-bottom: 12px; color: var(--text-primary);">
									<i class="fas fa-camera" style="color: var(--success); margin-right: 8px;"></i>
									Foto Progress <span style="color: #ef4444;">*</span>
								</label>
								<div style="display: grid; grid-template-columns: repeat(5, 1fr); gap: 16px;">
									@for($i = 1; $i <= 5; $i++)
										<div class="upload-box-wrapper" style="position: relative;">
											<div id="imagePreview_0_{{ $i }}" 
												class="upload-box"
												onclick="document.getElementById('uploadFile_0_{{ $i }}').click()"
												style="
													width: 100%; 
													aspect-ratio: 1; 
													...
													overflow: hidden;
												"
												onmouseover="this.style.borderColor='var(--success)'; this.style.background='linear-gradient(135deg, rgba(16, 185, 129, 0.1), rgba(6, 182, 212, 0.1))';"
												onmouseout="if(!this.classList.contains('has-image')){this.style.borderColor='rgba(16, 185, 129, 0.3)'; this.style.background='linear-gradient(135deg, rgba(16, 185, 129, 0.05), rgba(6, 182, 212, 0.05))';}"
											>
												<div class="upload-placeholder" style="text-align: center;">
													<i class="fas fa-cloud-upload-alt" style="font-size: 1.5rem; color: var(--success); margin-bottom: 4px;"></i>
											...
											<input type="file" id="uploadFile_0_{{ $i }}" name="fotoprogress{{ $i }}"
												accept="image/*" onchange="previewImagePremium(this, 0, {{ $i }})"
												style="display: none;"
												@if($i == 1) required @endif>
											@if($i == 1)
												<div style="position: absolute; top: -8px; right: -8px; background: linear-gradient(135deg, #ef4444, #dc2626); color: white; font-size: 0.6rem; padding: 2px 6px; border-radius: 4px; font-weight: 600;">
													<i class="fas fa-star" style="font-size: 0.5rem;"></i>
												</div>
											@endif
										</div>
									@endfor
								</div>
								<p style="font-size: 0.85rem; color: var(--text-tertiary); margin-top: 12px; display: flex; align-items: center; gap: 8px;">
									<i class="fas fa-info-circle" style="color: var(--success);"></i>
									Minimal upload 1 foto sebagai bukti progress pekerjaan
								</p>
							</div>
						</div>

						<div class="modal-footer" style="padding: 16px 24px; border-top: 1px solid var(--border-primary); background: var(--bg-tertiary); border-radius: 0 0 16px 16px; display: flex; gap: 12px; justify-content: flex-end;">
							<button type="submit" class="btn-premium btn-premium-primary" style="padding: 12px 24px; background: var(--gradient-primary); border: none; border-radius: 8px; color: white; font-weight: 600; cursor: pointer; display: inline-flex; align-items: center; gap: 8px;">
								<i class="fas fa-save"></i> Simpan
							</button>
							<button type="button" onclick="closeTambahModal()" style="padding: 12px 24px; background: var(--bg-secondary); border: 1px solid var(--border-primary); border-radius: 8px; color: var(--text-secondary); font-weight: 600; cursor: pointer;">
								Batal
							</button>
						</div>
					</form>
		...
		@foreach($laporanprogress as $progress)
			<div class="modal fade" id="modalEditProgress{{ $progress->id_progress }}" tabindex="-1" style="z-index: 1060;">
				<div class="modal-dialog" style="width: 90%; max-width: 800px; margin: 50px auto;">
					<div class="modal-content" style="background: var(--bg-card); border: 1px solid var(--border-primary); border-radius: 16px; box-shadow: 0 25px 50px -12px rgba(0,0,0,0.5);">
						<form action="{{ url('riwayatpemesanan/' . $value->id_pemesanan . '/update') }}" method="POST"
							...
							<div class="modal-header" style="padding: 20px 24px; border-bottom: 1px solid var(--border-primary); background: var(--bg-tertiary); border-radius: 16px 16px 0 0; display: flex; justify-content: space-between; align-items: center;">
								<h4 class="modal-title" style="margin: 0; font-size: 1.25rem; display: flex; align-items: center; gap: 10px;">
									<i class="fas fa-edit" style="color: var(--success);"></i> Ubah Progress
								</h4>
								<button type="button" class="close" data-dismiss="modal" style="background: var(--bg-secondary); border: none; width: 36px; height: 36px; border-radius: 8px; font-size: 1.5rem; color: var(--text-secondary); cursor: pointer;">&times;</button>
							</div>
...
<div style="display: grid; grid-template-columns: repeat(5, 1fr); gap: 16px;">
	...
	<div id="deleteProgressModal" style="display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; z-index: 9999; align-items: center; justify-content: center;">
		<div style="position: absolute; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0, 0, 0, 0.7); backdrop-filter: blur(4px);" onclick="hideDeleteProgressModal()"></div>
		<div style="position: relative; background: var(--bg-card); border: 1px solid var(--border-primary); border-radius: 16px; padding: 32px; max-width: 420px; width: 90%; text-align: center; box-shadow: 0 25px 50px -12px rgba(0, 0, 0, 0.5); animation: modalSlideIn 0.3s ease;">
			<div style="width: 80px; height: 80px; background: rgba(239, 68, 68, 0.15); border-radius: 50%; display: flex; align-items: center; justify-content: center; margin: 0 auto 20px;">
				<i class="fas fa-exclamation-triangle" style="font-size: 2.5rem; color: #ef4444;"></i>
			</div>
			<h3 style="font-size: 1.5rem; font-weight: 700; margin-bottom: 12px; color: var(--text-primary);">Hapus Progress</h3>
			<p style="color: var(--text-secondary); margin-bottom: 8px;">Apakah Anda yakin ingin menghapus progress ini?</p>
			<p id="deleteProgressInfo" style="font-weight: 600; color: var(--text-primary); padding: 12px 16px; background: var(--bg-tertiary); border-radius: 8px; margin-bottom: 24px;"></p>
			<div style="display: flex; gap: 12px; justify-content: center;">
				<button type="button" onclick="hideDeleteProgressModal()" style="padding: 12px 24px; background: var(--bg-tertiary); border: 1px solid var(--border-primary); border-radius: 8px; color: var(--text-secondary); font-weight: 600; cursor: pointer; display: flex; align-items: center; gap: 8px;">
					<i class="fas fa-times"></i> Batal
				</button>
				<button type="button" id="confirmDeleteProgressBtn" style="padding: 12px 24px; background: linear-gradient(135deg, #ef4444, #dc2626); border: none; border-radius: 8px; color: white; font-weight: 600; cursor: pointer; display: flex; align-items: center; gap: 8px; transition: all 0.2s ease;">
...
@section('scripts')
	<script>
		// Robust modal close functions
		function closeTrackingModal() {
			var modal = document.getElementById('trackingProgressModal');
			if (modal) {
		...
		// Delete progress modal functions
		var currentDeleteFormId = null;

		function showDeleteProgressModal(formId, progressInfo) {
			currentDeleteFormId = formId;
			document.getElementById('deleteProgressInfo').textContent = 'Progress: ' + progressInfo;
			var modal = document.getElementById('deleteProgressModal');
		...
		document.getElementById('confirmDeleteProgressBtn').addEventListener('click', function() {
		...
		document.addEventListener('keydown', function(e) {
			if (e.key === 'Escape' && document.getElementById('deleteProgressModal').style.display === 'flex') {
				hideDeleteProgressModal();
			}
		});

		function closeTambahModal() {
			var modal = document.getElementById('modalTambahProgress');
			if (modal) {
				modal.classList.remove('show', 'in');
				modal.style.display = 'none';
				modal.setAttribute('aria-hidden', 'true');
			}
			// Keep body open if tracking modal is still visible
			var trackingModal = document.getElementById('trackingProgressModal');
			if (trackingModal && trackingModal.classList.contains('show')) {
				// Re-ensure backdrop for tracking modal
		...
		// Premium preview with hidden placeholder
		function previewImagePremium(input, progressId, index) {
			if (input.files && input.files[0]) {
				var reader = new FileReader();
				reader.onload = function (e) {
					var preview = document.getElementById('imagePreview_' + progressId + '_' + index);
		...
		// Fix for nested modals - ensure backdrop stays when child modal closes
		$('#modalTambahProgress').on('hidden.bs.modal', function () {
			// Check if tracking modal is still open
			if ($('#trackingProgressModal').hasClass('show')) {
				// Re-add modal-open class to body
		...
		// When tracking modal opens after child modal was open
		$('#trackingProgressModal').on('show.bs.modal', function () {
			// Remove any orphaned backdrops
			$('.modal-backdrop').remove();
		});

		// Ensure clean state when tracking modal closes
		$('#trackingProgressModal').on('hidden.bs.modal', function () {
