...
@section('content')
    <div class="container" style="padding: 2rem 1rem;">
        ...
        {{-- Pesanan Aktif Section for Tukang --}}
        @if(Auth::user()->statuspengguna == '2' && isset($pesananAktif) && count($pesananAktif) > 0)
            <div class="active-orders animate-fadeIn" style="margin-bottom: var(--space-8);">
                <div class="orders-header"
                    style="display: flex; align-items: center; justify-content: space-between; padding: var(--space-4) var(--space-5); background: var(--bg-tertiary); border: 1px solid var(--border-primary); border-radius: var(--radius-xl) var(--radius-xl) 0 0;">
                    <h3 style="display: flex; align-items: center; gap: var(--space-2); font-size: 1rem; margin: 0;">
                        <i class="fas fa-calendar-check" style="color: var(--success);"></i> Pesanan Aktif
                    </h3>
                    <span style="font-size: 0.85rem; color: var(--text-tertiary);">{{ count($pesananAktif) }} pesanan</span>
                </div>
                <div
                    style="background: var(--bg-card); border: 1px solid var(--border-primary); border-top: none; border-radius: 0 0 var(--radius-xl) var(--radius-xl); overflow-x: auto;">
                    <table style="width: 100%; border-collapse: collapse;">
                        <thead>
                            <tr style="background: var(--bg-secondary);">
                                <th
                                    style="padding: var(--space-4); text-align: left; font-size: 0.8rem; font-weight: 600; color: var(--text-tertiary); text-transform: uppercase;">
                                    No</th>
                                <th
                                    style="padding: var(--space-4); text-align: left; font-size: 0.8rem; font-weight: 600; color: var(--text-tertiary); text-transform: uppercase;">
                                    Nomor Pemesanan</th>
                        ...
                        <tbody>
                            @php $i = 1; @endphp
                            @foreach($pesananAktif as $value)
                                <tr style="border-bottom: 1px solid var(--border-primary);">
                                    <td style="padding: var(--space-4);">{{ $i }}</td>
                                    <td style="padding: var(--space-4);"><span
                                            style="font-weight: 600; color: var(--success);">{{ $value->nomorpemesanan }}</span>
                                    </td>
                                    <td style="padding: var(--space-4);">{{ $value->namapelanggan ?? '-' }}</td>
                                    <td style="padding: var(--space-4);">{{ $value->jenispemesanan ?? '-' }}</td>
                                    <td style="padding: var(--space-4);">{{ $value->tanggalbekerja }}</td>
                                    <td style="padding: var(--space-4);">
                                        <a href="{{ url('riwayatpemesanan') }}/{{ $value->id_pemesanan }}"
                    ...
                    <div class="order-card animate-fadeIn" style="animation-delay: {{ $key * 0.1 }}s;">
                        <div class="order-header">
                            <div class="order-number">
                                ...
                                <a
                                    href="{{ url('riwayatpemesanan') }}/{{ $value->id_pemesanan }}@if(Auth::user()->statuspengguna == '1')?kategori=all&katakunci=@endif">
