                    ...
                    <form action="" method="GET" class="filter-form">
                        ...
                        <div class="filter-group">
                            ...
                            <select class="form-control" name="jarak">
                                <option value="5" {{ isset($_GET['jarak']) && $_GET['jarak'] == 5 ? 'selected' : (!isset($_GET['jarak']) ? 'selected' : '') }}>5 Km</option>
                                <option value="10" {{ isset($_GET['jarak']) && $_GET['jarak'] == 10 ? 'selected' : '' }}>10 Km</option>
                                <option value="15" {{ isset($_GET['jarak']) && $_GET['jarak'] == 15 ? 'selected' : '' }}>15 Km</option>
                            </select>
                        </div>
...
function initMap() {
    ...
    // Cek apakah sudah pernah memilih di sesi ini
    let lokasiChoice = sessionStorage.getItem('lokasiChoice');
    
    if (lokasiChoice === null) {
        // Belum pernah memilih, tampilkan popup
        let pilihan = confirm(
            "Gunakan lokasi GPS perangkat?\n\nOK = GPS\nCancel = Lokasi dari Pengaturan Akun"
        );
        
        // Simpan pilihan di sessionStorage
        sessionStorage.setItem('lokasiChoice', pilihan ? 'gps' : 'db');
        lokasiChoice = pilihan ? 'gps' : 'db';
    }
