...
Route::middleware(['auth'])->group(function () {
	...
	Route::get('konfirmasi-update-saldo', function (Request $request) {
		$updatesaldo = \App\RiwayatTransaksi::select('riwayattransaksi.*', 'users.nomorrekening', 'users.namarekening', 'pelanggan.namapelanggan')
			->distinct()
			->join('users', 'users.id', '=', 'riwayattransaksi.id')
	...
	Route::get('permintaan-pesanan', function (Request $request) {
		...
		$permintaan = \App\Pemesanan::join('kategoritukang', 'kategoritukang.id_kategoritukang', '=', 'pemesanan.id_kategoritukang')->join('pelanggan', 'pelanggan.id_pelanggan', '=', 'pemesanan.id_pelanggan')->join('users', 'users.id', '=', 'pelanggan.id')->where('id_tukang', '=', $id_tukang)->where('statuspemesanan', '=', '0')->orderby('id_pemesanan', 'DESC')->get();
