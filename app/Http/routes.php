...
Route::group(['middleware' => 'auth'], function () {
	...
	Route::resource('datajenispemesanan', 'DataJenisPemesananController');

	Route::resource('datakategoritukang', 'DataKategoriTukangController');

	Route::get('databahanmaterial/{id}/ubahstatus', 'DataBahanMaterialController@ubahstatus');
	...
	Route::post('ubahhargajarak',function(){
		$hargajarak = \App\HargaJarak::find("1");
		$hargajarak->hargajarak = Input::get('hargajarak');
		$hargajarak->save();
	...
	Route::post('konfirmasiupdatesaldo/terima',function(){
		$terima = \App\RiwayatTransaksi::find(Input::get('idriwayat'));
	...
	Route::post('konfirmasiupdatesaldo/tolak',function(){
		$terima = \App\RiwayatTransaksi::find(Input::get('idriwayat'));
	...
	Route::post('konfirmasitariksaldo/terima',function(){
		$terima = \App\RiwayatTransaksi::find(Input::get('idriwayat'));
		$terima->statustransaksi = "1";
		$terima->save();
		$notifikasi = new \App\Notifikasi;
	...
	Route::post('konfirmasitariksaldo/tolak',function(){
		$terima = \App\RiwayatTransaksi::find(Input::get('idriwayat'));
		$terima->statustransaksi = "2";
		$user = \App\User::find($terima->id);
		$user->saldo += $terima->jumlahsaldo;
	...
	Route::get('riwayatpemesanan/{id}/lihatpeta',function($id){
		function haversineGreatCircleDistance($latitudeFrom, $longitudeFrom, $latitudeTo, $longitudeTo, $earthRadius = 6371000)
		{
			$latFrom = deg2rad($latitudeFrom);
			$lonFrom = deg2rad($longitudeFrom);
	...
	Route::get('pengaturanjasakeahlian',function(){
		...
		return View::make('pengaturanjasadankeahlian')->with(['jenispemesanan'=>$jenispemesanan,'jasatersediaharian'=>$jasatersediaharian,'jasatersediaborongan'=>$jasatersediaborongan]);
	});
	Route::get('notifikasi/{id}/markasread',function($id){
	...
	Route::post('pengaturanjasakeahlian',function(){
		if(Input::get('kurikulum') != "" && Input::get('kurikulum2') != "")
		{
			...
			$kurikulum3 = "";
			for($i=0;$i<count(Input::get('kurikulum'));$i++)
			{
				$jasatersedia = new \App\JasaTersedia;
				$jasatersedia->id_jenispemesanan = Input::get('kurikulumjp')[$i];
				$jasatersedia->biayajasatersedia = Input::get('kurikulum')[$i];
				$jasatersedia->jenisjasatersedia = '0';
				$jasatersedia->id_tukang = $user->id_tukang;
				$jasatersedia->save();
			}
			for($i=0;$i<count(Input::get('kurikulum2'));$i++)
			{
				$jasatersedia = new \App\JasaTersedia;
				$jasatersedia->id_jenispemesanan = Input::get('kurikulumjp2')[$i];
				$jasatersedia->biayajasatersedia = Input::get('kurikulum2')[$i];
				$jasatersedia->jenisjasatersedia = '1';
				$jasatersedia->id_tukang = $user->id_tukang;
				$jasatersedia->save();
			}
			for($i=0;$i<count(Input::get('kurikulum3'));$i++)
			{
				if($kurikulum3 == "")
				{
					$kurikulum3 = Input::get('kurikulum3')[$i];
				}
				else
				{
					$kurikulum3 = $kurikulum3 . "~" . Input::get('kurikulum3')[$i];
				}
			}
			$simpan->pengalamanbekerja = $kurikulum3;
			$simpan->lamapengalamanbekerja = Input::get('pengalaman');
			$simpan->deskripsikeahlian = Input::get('deskripsi');
	...
	Route::post('pengaturanakun',function(){
		if(Input::get('latitude') != "")
		{
			...
			if(Auth::user()->statuspengguna == '2')
			{
				...
				$tukang->namatukang = Input::get('name');
				$tukang->statuseditprofil = '1';
				if(Input::hasFile('fotoprofil'))
				{
					$fotoprofil = 'fotoprofil' . Auth::user()->id . '.jpg';
					Input::file('fotoprofil')->move('images/fotoprofil',$fotoprofil);
			...
			else
			{
				...
				$pelanggan->namapelanggan = Input::get('name');
				if(Input::hasFile('fotoprofil'))
				{
					$fotoprofil = 'fotoprofil' . Auth::user()->id . '.jpg';
					Input::file('fotoprofil')->move('images/fotoprofil',$fotoprofil);
			...
			$users->nomorhandphone = Input::get('nomorhandphone');
			$users->nomorrekening = Input::get('nomorrekening');
			$users->namarekening = Input::get('namarekening');
			$users->latitude = Input::get('latitude');
			$users->longtitude = Input::get('longtitude');
	...
	Route::get('caritukang', function () {
	    ...
	    // INPUT FILTER
	    // ===============================
	    $kategori = Input::get('kategori'); // id atau 'all'
	    $radius   = Input::get('jarak') ? Input::get('jarak') * 1000 : 5000;
	    $nama     = Input::get('nama');
	...
	Route::get('caritukang/{idtukang}/rincianbiaya',function($idtukang){
		...
		return View::make('detailtukangrincianbiaya')->with(['idtukang'=>$idtukang,'tukang'=>$tukang,'jasatersediaharian'=>$jasatersediaharian,'jasatersediaborongan'=>$jasatersediaborongan,'alamatpelanggan'=>$alamatpelanggan]);
	});
	Route::get('caritukang/{idtukang}/pengalamanbekerja',function($idtukang){
		...
		$jasatersediaborongan = \App\JasaTersedia::join('jenispemesanan','jenispemesanan.id_jenispemesanan','=','jasatersedia.id_jenispemesanan')->where('id_tukang','=',$idtukang)->where('jenisjasatersedia','=','1')->get();
		$alamatpelanggan = \App\AlamatPelanggan::where('id_pelanggan','=',Auth::user()->id_pelanggan)->get();
		return View::make('detailtukangpengalamanbekerja')->with(['idtukang'=>$idtukang,'tukang'=>$tukang,'jasatersediaharian'=>$jasatersediaharian,'jasatersediaborongan'=>$jasatersediaborongan,'alamatpelanggan'=>$alamatpelanggan]);
	});
	Route::get('caritukang/{idtukang}/deskripsikeahlian',function($idtukang){
		...
		$jasatersediaborongan = \App\JasaTersedia::join('jenispemesanan','jenispemesanan.id_jenispemesanan','=','jasatersedia.id_jenispemesanan')->where('id_tukang','=',$idtukang)->where('jenisjasatersedia','=','1')->get();
		$alamatpelanggan = \App\AlamatPelanggan::where('id_pelanggan','=',Auth::user()->id_pelanggan)->get();
		return View::make('detailtukangdeskripsikeahlian')->with(['idtukang'=>$idtukang,'tukang'=>$tukang,'jasatersediaharian'=>$jasatersediaharian,'jasatersediaborongan'=>$jasatersediaborongan,'alamatpelanggan'=>$alamatpelanggan]);
	});
	Route::get('caritukang/{idtukang}/komentarpelanggan',function($idtukang){
		...
		return View::make('detailtukangkomentarpelanggan')->with(['idtukang'=>$idtukang,'tukang'=>$tukang,'jasatersediaharian'=>$jasatersediaharian,'jasatersediaborongan'=>$jasatersediaborongan,'alamatpelanggan'=>$alamatpelanggan,'jumlahkomentar'=>$jumlahkomentar,'ulasan'=>$ulasan]);
	});
	Route::post('caritukang/{idtukang}/komentarpelanggan',function($idtukang){
		$rating = new \App\Ulasan;
		$tukang = \App\Tukang::find($idtukang);
		$pemesanan = \App\Pemesanan::find(Input::get('idpemesanan'));
		$pemesanan->statuspemesanan = "5";
		$pemesanan->save();
		$notifikasi = new \App\Notifikasi;
		$tukang->rating = ($tukang->totalvote + Input::get('nilairating'))/($tukang->jumlahvote+1);
		$tukang->jumlahvote += 1;
		$tukang->totalvote += Input::get('nilairating');
		$tukang->save();
		$rating->id_tukang = $idtukang;
		$rating->id_pelanggan = Auth::user()->id_pelanggan;
		$rating->rating = Input::get('nilairating');
		$rating->isiulasan = Input::get('isiulasan');
		$rating->save();
		$notifikasi->isinotifikasi = "telah memberikan ulasan terhadap jasa anda dengan nilai rating " . Input::get('nilairating');
	...
	Route::get('caritukang/{idtukang}/lokasi',function($idtukang){
		...
		$jasatersediaborongan = \App\JasaTersedia::join('jenispemesanan','jenispemesanan.id_jenispemesanan','=','jasatersedia.id_jenispemesanan')->where('id_tukang','=',$idtukang)->where('jenisjasatersedia','=','1')->get();
		$alamatpelanggan = \App\AlamatPelanggan::where('id_pelanggan','=',Auth::user()->id_pelanggan)->get();
		return View::make('detailtukanglokasi')->with(['idtukang'=>$idtukang,'tukang'=>$tukang,'jasatersediaharian'=>$jasatersediaharian,'jasatersediaborongan'=>$jasatersediaborongan,'alamatpelanggan'=>$alamatpelanggan]);
	});
	Route::post('caritukang/{idtukang}/pesan',function($idtukang){
		function quickRandom($length)
		{
		    $pool = '0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ';
		    return substr(str_shuffle(str_repeat($pool, 5)), 0, $length);
		}
		if(Input::get('jenis')=="0")
			$hasilpotonganjp = explode(",", Input::get('jenispemesanan'));
		else
			$hasilpotonganjp = explode(",", Input::get('jenispemesanan2'));
		...
		else
		{
			...
			$pesan->tanggalbekerja = Input::get('tanggalbekerja');
			if(Input::get('jenis')!="0")
				$pesan->tanggalselesai = Input::get('tanggalselesai');
			$pesan->catatan = Input::get('catatan');
			$pesan->kategoripemesanan = Input::get('jenis');
			if(Input::hasFile('foto1'))
			{
				$fotoproduk1 = 'fotopesan1' . date('YmdHis') . '.jpg';
				Input::file('foto1')->move('images/fotoproduk',$fotoproduk1);
				$pesan->fotopemesanan1 = $fotoproduk1;
			}
			if(Input::hasFile('foto2'))
			{
				$fotoproduk2 = 'fotopesan2' . date('YmdHis') . '.jpg';
				Input::file('foto2')->move('images/fotoproduk',$fotoproduk2);
				$pesan->fotopemesanan2 = $fotoproduk2; 
			}
		    $hasilpotongan = explode(",", Input::get('alamatpemesanan'));
	...
	Route::post('tambahalamat',function(){
		if(Input::get('latitude') == Auth::user()->latitude)
		{
		...
		else
		{
			...
			$alamatpelanggan->alamatpelanggan = Input::get('alamat');
			$alamatpelanggan->latitudealamat = Input::get('latitude');
			$alamatpelanggan->longtitudealamat = Input::get('longtitude');
	...
	Route::post('tambahalamat/{idalamat}',function($idalamat){
	...
	Route::post('isisaldo',function(){
		...
		if(Input::get('jumlahsaldouser') >= 100000)
		{
			$kode = "KT" . quickRandom(3);
		    $saldo->kode = $kode;
		    $saldo->id = Auth::user()->id;
			$jumlahsaldo = Input::get('jumlahsaldouser');
			$saldo->jumlahsaldo = $jumlahsaldo;
		   	$saldo->rekening = Input::get('nomorrekeninganda');
		   	$saldo->namarekening = Input::get('namapemilik');
		    $saldo->rekeningtujuan = Input::get('nomorrekening');
		    $saldo->jenistransaksi = "Pengisian Saldo";
		    $saldo->statustransaksi = "0";
		    $buktitransfer = 'buktitransfer' . $kode . '.jpg';
			Input::file('buktitransfer')->move('images/buktitransfer',$buktitransfer);
	...
	Route::get('notifikasi',function(){
	    ...
	    return View::make('notifikasi')->with(['notifread'=>$notifread,'notifunread'=>$notifunread]);
	});
	Route::get('permintaanpesanan',function(){
		...
		return View::make('permintaanpesanan')->with(['pesanan'=>$pesanan,'permintaan'=>$permintaan]);
	});
	Route::post('permintaanpesanan/{idpemesanan}/tolak',function($idpemesanan){
		function quickRandom($length)
		{
		    $pool = '0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ';
		    return substr(str_shuffle(str_repeat($pool, 5)), 0, $length);
		...
		$tolak->alasanpenolakanpemesanan = Input::get('alasanpenolakan');
		$tolak->save();
	...
	Route::post('permintaanpesanan/{idpemesanan}/terima',function($idpemesanan){
		function quickRandom($length)
		{
		    $pool = '0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ';
		    return substr(str_shuffle(str_repeat($pool, 5)), 0, $length);
	...
	Route::post('penarikansaldo',function(){
		function quickRandom($length)
		{
		    $pool = '0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ';
		    return substr(str_shuffle(str_repeat($pool, 5)), 0, $length);
		}
		if(Input::get('jumlahsaldouser') <= Auth::user()->saldo)
		{
			...
			$saldo->kode = $kode;
		    $saldo->id = Auth::user()->id;
			$jumlahsaldo = Input::get('jumlahsaldouser')-(Input::get('jumlahsaldouser')*5/100);
			$saldo->jumlahsaldo = $jumlahsaldo;
		    ...
		    $saldo->statustransaksi = "0";
		    $saldo->save();
		    $users->saldo -= Input::get('jumlahsaldouser');
		    $users->save();
	...
	Route::get('riwayatpemesanan',function(){
		if(Auth::user()->statuspengguna == '2')
		{
			...
			return View::make('riwayatpemesanan')->with(['riwayatpemesanan'=>$riwayatpemesanan]);
		}
		...
		elseif(Auth::user()->statuspengguna == '0')
		{
			...
			return View::make('adminriwayatpemesanan')->with(['riwayatpemesanan'=>$riwayatpemesanan]);
		}
	});
	Route::get('riwayatpemesanan/{idpemesanan}',function($idpemesanan){
		$hasilpencarian = "";
		$pemesananbahan = "";
		$totalkeranjang = "0";
		$statuspemesanan = "";
		...
		if(Auth::user()->statuspengguna == '2')
		{
			...
			$hargajarak = \App\HargaJarak::find("1");
			$statuspemesanan = $value->statuspemesanan;
			return View::make('detailriwayatpemesanan')->with(['value'=>$value,'jarak'=>$jarak,'hasilpencarian'=>$hasilpencarian,'pemesananbahan'=>$pemesananbahan,'idpemesanan'=>$idpemesanan,'totalkeranjang'=>$totalkeranjang,'statuspemesanan'=>$statuspemesanan,'hargajarak'=>$hargajarak,'laporanprogress'=>$laporanprogress]);
		}
		elseif(Auth::user()->statuspengguna == '1')
		{
			...
			// Kirim data ke view
			return View::make('detailriwayatpemesanan')->with([
			    'value' => $value,
			    'jarak' => $jarak,
		...
		elseif(Auth::user()->statuspengguna == '0')
		{
			...
			return View::make('admindetailriwayatpemesanan')->with(['value'=>$value,'jarak'=>$jarak,'pemesananbahan'=>$pemesananbahan,'idpemesanan'=>$idpemesanan,'totalkeranjang'=>$totalkeranjang,'statuspemesanan'=>$statuspemesanan,'tukang'=>$tukang,'hargajarak'=>$hargajarak,'laporanprogress'=>$laporanprogress]);
		}
	});
	Route::post('riwayatpemesanan/{idpemesanan}/{idbahanmaterial}/masukkeranjang',function($idpemesanan,$idbahanmaterial){
		...
		$pemesananbahan->qtypembelian = Input::get('qty');
		$pemesananbahan->hargapemesananbahanmaterial = Input::get('hargapemesanan');
	...
	Route::get('riwayatpemesanan/{idpemesanan}/{idpemesananbahanmaterial}/hapus',function($idpemesanan,$idpemesananbahanmaterial){
		$pemesanan = \App\Pemesanan::find($idpemesanan);
	...
	Route::post('riwayatpemesanan/{idpemesanan}/prosespembelian',function($idpemesanan){
		if(Auth::user()->saldo < Input::get('totalkeranjang')+Input::get('biayajarak'))
		{
			$pemesanan = \App\Pemesanan::find($idpemesanan);
		...
		else
		{
			...
			$users = \App\User::find($pelanggan->id);
		    $users->saldo -= Input::get('totalkeranjang')+Input::get('biayajarak');
		    $users->save();
	...
	Route::post('riwayatpemesanan/{idpemesanan}/selesaidikerjakan',function($idpemesanan){
		function quickRandom($length)
		{
		    $pool = '0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ';
		    return substr(str_shuffle(str_repeat($pool, 5)), 0, $length);
	    ...
	    $users->saldo += Input::get('biayajasa');
	    $users->save();
	    $notifikasi = new \App\Notifikasi;
		$notifikasi->kepada = $tukang->id;
	    ...
	    $saldo->kode = $kode;
	    $saldo->id = $tukang->id;
		$jumlahsaldo = Input::get('biayajasa');
		$saldo->jumlahsaldo = $jumlahsaldo;
	...
	Route::post('riwayatpemesanan/{idpemesanan}/ubahbiaya',function($idpemesanan){
		$pemesanan = \App\Pemesanan::find($idpemesanan);
		$pelanggan = \App\Pelanggan::find($pemesanan->id_pelanggan);
		$user = \App\User::find($pelanggan->id);
		$biaya = Input::get('biayajasaubah') - $pemesanan->biayajasa;
		$user->saldo -= $biaya;
		$user->save();
		$pemesanan->biayajasa = Input::get('biayajasaubah');
	...
	Route::post('riwayatpemesanan/{idpemesanan}/izinkan',function($idpemesanan){
		$pemesanan = \App\Pemesanan::find($idpemesanan);
	...
	Route::get('detailriwayatpemesanan',function(){
...
Route::get('auth/registertukang',function(){
    return View::make('auth/registertukang');
});
Route::controllers([
